        ...
        with archive.open(filename) as f:
            ...
            for line in f:
                ...
                for hashtag in hashtags:
                    lang = tweet['lang']
                    country = 'N/A'
                    # get country if country code is defined

                    # More thorough check that yields the same result
                    '''
                    if ('place' in tweet and
                            tweet['place'] is not None and
                            'country_code' in tweet['place'] and
                            tweet['place']['country_code'] is not None and
                            tweet['place']['country_code'] != ""):
                        country = tweet['place']['country_code']
                    '''

                    if (tweet['place'] is not None and
                            tweet['place']['country_code'] != ""):
                        country = tweet['place']['country_code']
                    if hashtag in text:
